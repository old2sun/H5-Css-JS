...
(function(window,undefined){
	...
	var Jerry = function(selector,context){
			return new Jerry.fn.init(selector,context);
		}
		Jerry.fn = Jerry.prototype = {
			constructor : Jerry,  // 构造函数指回Jerry(然而并没有卵用)
			init : function(selector,context){
				...
				if(typeof selector === 'string'){
					...
					if(selector.charAt(0)==='<'){
							...
							// 获得元素集合
							// 这里的 this 指向 init实例
							[].push.apply(this,Jerry.select(selector,context));
						}
						// 给实例对象赋selector属性,这是一个标志类,就是实例化后，这些数据肯定是实例对象(个人理解)
	...
	Jerry.fn.extend({
		css : function(){
			...
			if(p.length === 1){
				...
				else if(Jerry.isObj(p[0])){
					// 判断 实参是否对象
					return this.each(function(){
						// 遍历对象
						for(var k in p[0]){
							// 这里的this为每个单独的DOM元素,p[0][k],obj[0] obj[1]
							this.style[k] = p[0][k];
						}
					});
				}else if(Jerry.isArray(p[0])){
					// 判断是否为数组  p[0]   ['width',height]
					var obj = {}; // 数组转换位类数组{'width':'100px','height':'100'px}
					for(var i=0;i<p[0].length;i++){
						// p[0] 是数组,转到对象 obj{}，但是获取值的是 getStyle，与obj无关
				...
				return this.each(function(){
					// this 为单个   
					// css('width','100px')   p[0]第一个实参，p[1]第二个实参
					this.style[p[0]] = p[1];
				});
			}
		},
		hasClass : function(cn){
			...
			var arr = cname.split(/s+/g); // 按空格分隔类名，放进数组
			for(var i=0;i<arr.length;i++){
				if(arr[i] === cn){
					return true // 有此类名为true
				}
			}
			return false; //没有此类名为false
		},
		addClass : function(){
			// 利用实参属性
			var p = arguments;
		...
		removeClass : function(){
			var p = arguments;
			return this.each(function(){
				var arr = p[0].split(/\s+/g); // 按空格分隔,返回一个数组
				for(var i=0;i<arr.length;i++){
					// 循环每个 元素的类名
					var className = this.className;
					//  匹配类名， \b 为单词边界
					var reg = new RegExp('\\b'+Jerry.trim(arr[i])+'\\b','g');
					// 另一种写法
					// className = className.replace(Jerry.trim(p[0]),' ');
					//  删除匹配的名字
		...
		// 	var p = arguments; // 实参是数组
		// 	if(p.length == 1){
		// 		if(Jerry.isString(p[0])){
		// 			var reg = /\b\w+\b/g;  // 匹配单词表达式
		// 			var arr = p[0].match(reg);  // 匹配为数组
		// 			console.dir(arr);
		// 			if(arr.length === 1){
		// 				// 有一个类时
		...
		// 		}
		// 	}
		// 	else if(p.length === 2){ // 参数为 函数与 boolean
		// 		this.each(function(i,element){
		// 			if(Jerry.isFunction(p[0])){
		// 				var ret = p[0].call(this,i,this.className,p[1]);
		// 				// 调用传过来的函数，this 为单个实例
		// 				this.className = ret; 
		// 			}
		// 		});
	...
	Jerry.fn.extend({
		on : function(type,fn,flag){
			var arg = arguments;
			if(arg.length===1 && Jerry.isObj(arg[0])){
				// 判断传入的是否对象
				for(var key in arg[0]){
					...
					this.each(function(){
						if(this.addEventListener){
							// this 单个元素
							this.addEventListener(key,arg[0][key],false);
						}else{
							this.attachEvent('on'+key,arg[0][key]); // 兼容 IE
						}
					});
				}
			}
				...
				var reg = /\b\w+\b/g;
				var arr = arg[0].match(reg);
				for(var i=0;i<arr.length;i++){
					flag = flag?flag:false;
					this.each(function(){
						if(this.addEventListener){
							this.addEventListener(arr[i],fn,flag);
						}else{
							this.attachEvent('on'+arr[i],fn);
						}
					});
				}
			}
			...
			for(var i=0;i<arr.length;i++){
				this.each(function(){
					if(this.removeEventListener){
						this.removeEventListener(arr[i],fn,flag);
					}else{
	...
	Jerry.extend({
        ...
        ajax : function(param){
            ...
            // 给默认对象赋新的值
            Jerry.extend(defaults,param);
            // 判断是发送Ajax请求，还是进行跨域请求(通过script的src属性发送请求)
            if(defaults.dataType==='json'){
                jsonFn(defaults);
            }else if(defaults.dataType==='jsonp'){
                jsonpFn(defaults);
            }
            ...
            function jsonFn(p){
                ...
                // 执行发送动作,此时如果方式为get,param为空;如果为post，需要传param参数
                xhr.send(param);
                // 指定回调函数
            ...
            function jsonpFn(p){
                //  跨域请求回调函数的默认名  jQuery11210757162399502803_1466185818031
                var cbName = 'jQuery' + ('1.12.1' + Math.random()).replace(/\D+/g,'') + '_' + (new Date().getTime());
                if(p.jsonpCallback){
                    // 自定义回调函数的名字
                    cbName = p.jsonpCallback;
                }
                //这个是回调函数(意思是在window对象中添加一个方法,与obj[name]类似)
